...
class BootOMetaGrammar(GrammarBase):
    ...
    def rule_namedChar(self):
        _locals = {'self': self}
        self.locals['namedChar'] = _locals
        _G_exactly_1, lastError = self.exactly('{')
        self.considerError(lastError)
        def _G_many1_2():
        ...
        self.considerError(lastError)
        _locals['c'] = _G_many1_3
        _G_exactly_4, lastError = self.exactly('}')
        self.considerError(lastError)
        _G_python_5, lastError = eval("self.builder.get_named_character(''.join(c))", self.globals, _locals), None
        self.considerError(lastError)
    ...
    def rule_character(self):
        ...
        def _G_many_3():
            def _G_or_1():
                ...
                return (_G_apply_1, self.currentError)
            def _G_or_2():
                _G_apply_1, lastError = self._apply(self.rule_namedChar, "namedChar", [])
                self.considerError(lastError)
                return (_G_apply_1, self.currentError)
            def _G_or_3():
                def _G_not_1():
    ...
    def rule_comment(self):
        ...
        self.considerError(lastError)
        _G_apply_8, lastError = self._apply(self.rule_vspace, "vspace", [])
        self.considerError(lastError)
    ...
    def rule_rule(self):
        ...
        _G_many_2, lastError = self.many(_G_many_1)
        self.considerError(lastError)
        _G_apply_3, lastError = self._apply(self.rule_noindentation, "noindentation", [])
        self.considerError(lastError)
        def _G_lookahead_4():
        ...
        self.considerError(lastError)
        _G_python_6, lastError = eval('n', self.globals, _locals), None
        self.considerError(lastError)
        _G_apply_7, lastError = self._apply(self.rule_rulePart, "rulePart", [_G_python_6])
        self.considerError(lastError)
        _locals['r'] = _G_apply_7
        def _G_or_8():
            def _G_many1_1():
